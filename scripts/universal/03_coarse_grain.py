...
def get_best_structure(run_dir):
    ...
    if not os.path.exists(best_model_link):
        # Use absolute path for robust linking
        os.symlink(os.path.abspath(pdb_path), best_model_link)
...
def run_martinize_docker(run_dir, pdb_path, config):
    ...
    def _select_optimal_force_field(seq: str) -> tuple[str, str]:
        ...
        n = len(seq)
        if n == 0:
            return 'martini3001', 'default (no sequence)'
        aromatic = sum(1 for aa in seq if aa in 'FYW') / n
        if n < 10 and aromatic > 0.2:
            return 'martini22', 'short aromatic peptide (prefer martini22)'
        elif n < 20:
            return 'martini3001', 'short peptide (martini3 with standard settings)'
        return 'martini3001', 'default'

    force_field = cg_config.get('force_field', 'martini3001')
    if cg_config.get('force_field_auto', True):
    ...
    if cg_config.get('elastic_network', True):
        cmd.extend([
...
def _clash_fraction(coords, cutoff_nm=0.25):
    n = len(coords)
    if n < 2:
        return 0.0
    arr = np.vstack(coords)
...
def validate_cg_structure(run_dir, peptide_id, pdb_path, config=None):
    ...
    Checks:
      - CG clash fraction (pair distance < 0.25 nm)
      - CG backbone BB distances within [0.2, 0.7] nm
      - Rg deviation AA vs CG (< 20%)
    """
    ...
    thr = {
        ...
        'bb_dist_min_nm': float(vcfg.get('bb_dist_min_nm', 0.20)),
        'bb_dist_max_nm': float(vcfg.get('bb_dist_max_nm', 0.70)),
    ...
    # Clash fraction
    cf = _clash_fraction(cg_coords, cutoff_nm=0.25)
    ...
    if (rg_cg is not None) and (rg_aa is not None) and rg_aa > 1e-12:
    ...
    # Console summary
    print("\nCG validation:")
    print(f"  Clash fraction: {cf:.4f} (≤ {thr['clash_frac_max']})")
...
def print_summary(run_dir, peptide_id, config, metadata=None):
    ...
    elastic_network = config.get('coarse_graining', {}).get('elastic_network', True)
