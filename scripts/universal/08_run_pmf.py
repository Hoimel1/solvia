...
class PMFRunner:
    ...
    def __init__(self, run_dir, config=None):
        ...
        self.max_extend_ns = float(self.qc_config.get('max_extend_ns', 20.0))
    ...
    def run_smd_ladder(self, start_structure: Path, target_z: float, output_dir: Path,
        ...
        smd_time = min(float(self.umbrella_config.get('pre_smd_ns', 0.5)), max_smd)
...
"""
    ...
    def check_window_overlap(self, window1_pullx, window2_pullx, target_overlap=0.20):
        ...
        if method == 'kde':
            try:
                ...
                overlap = float(np.trapz(np.minimum(p1, p2), xs))
    ...
    def run_umbrella_window(self, window_center, window_dir, start_structure, 
        ...
        # Validate reference group consistency (warn if poor)
        try:
        ...
        if smd_prepare and float(self.umbrella_config.get('pre_smd_ns', 5.0)) > 0:
...
"""
    ...
    def run_pmf_calculation(self, replicate=1, tag=None, resume=False, qc_only=False):
        ...
        max_rounds = int(self.qc_config.get('max_qc_rounds', 3))
