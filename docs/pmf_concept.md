...
Nach Abschluss aller Umbrella-Simulationen für ein Peptid werden die Daten zusammengeführt, um das Freie-Energie-Profil G(z) zu berechnen:
	•	Auswertung mittels MBAR: Wir verwenden die Multistate Bennett Acceptance Ratio (MBAR) Methode, um aus den Umbrella-Histogrammen das kontinuierliche PMF zu bestimmen. MBAR ist ein moderner, effizienter Ansatz, der aus den kombinierten Daten aller Fenster die Zustandsfreie Energien selbstkonsistent berechnet. Zum Abgleich wird parallel auch klassisches WHAM (Weighted Histogram Analysis) berechnet, um sicherzustellen, dass beide zum gleichen Ergebnis gelangen (dient als QA-Check; primär berichten wir die MBAR-Resultate). Die Bulk-Wasser-Referenz wird so gewählt, dass G(z) im Bulk auf 0 kJ/mol normiert wird: konkret setzen wir den Mittelwert von G(z) für z ≥ +2,4 nm als Nullniveau (dort ist das Peptid vollständig gelöst, fern der Membran). Negative freie Energien bedeuten also eine energetische Bevorzugung des Peptids in Membrannähe verglichen mit der Bulk-Lösung.
...
• Analyse: MBAR primär; WHAM als QA; PCHIP (0.01 nm) nur zur Extrempunkt-Bestimmung; präzisionsgewichtete Replikat-Mittel.
